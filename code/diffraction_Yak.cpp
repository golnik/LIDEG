...
int main(int argc, char **argv)
{
    try
    {
        ...
        std::vector<complex_t> Q_mn(N_xyz);
        ...
        for (size_t izone = 0; izone < nzones; izone++)
        {
            for (auto spot : zones[izone])
            {
                             ...
                             &vecs, &graphene](const size_t &ikx, const size_t &iky)
                {
                    ...
                    for (size_t mst = 0; mst < Nst; mst++)
                    {
                        ...
                        for (size_t nst = 0; nst < Nst; nst++)
                        {
                            ...
                            for (size_t ix = 0; ix < params.Nx; ix++)
                            {
                                for (size_t iy = 0; iy < params.Ny; iy++)
                                {
                                    for (size_t iz = 0; iz < params.Nz; iz++)
                                    {
                                        ...
                                        complex_t Q_mstnst = std::conj(Psis[mst]) * Psis[nst];
                            ...
                            std::vector<complex_t> Q_xy_mn(N_xy);
                            ...
                            for (size_t ix = 0; ix < params.Nx; ix++)
                            {
                                for (size_t iy = 0; iy < params.Ny; iy++)
                                {
                                    ...
                                    std::complex<double> res_xy_mn = 0.;
                                    ...
                                    integrator_z->trapz(
                                        [m, n, ix, iy, &Q_mn, &indx_xyz, &zgrid, &params](const size_t &iz)
                                        {
                                            size_t indx_ixiyiz = indx_xyz({iz, iy, ix});
                                            double z = (*zgrid)[iz];
                                            double S_square = pow(2. * M_PI / params.a, 2.0) * (pow(1. / sqrt(3.) * (m + n), 2.0) + pow((m - n), 2.0));
                                            double kz_z = S_square * z / (2 * 47);
                                            std::complex<double> PW = exp(I * kz_z);

                                            return Q_mn[indx_ixiyiz] * PW;
                                        },
                                        res_xy_mn);
                            ...
                            integrator_xy->trapz([&Fourier_transform, &Q_xy_mn](const size_t &iy, const size_t &ix)
                                                 { return Fourier_transform(iy, ix, Q_xy_mn); }, F_S_mn);

                            if (mst == nst && mst == 0) // intra band
                            {
                                res_k[0] = res_k[0] + rho[mst][nst] * F_S_mn;
                            }

                            if (mst != nst) // inter band
                            {
                                res_k[1] = res_k[1] + rho[mst][nst] * F_S_mn;
                            }

                            // total
                            // res_k[2] = res_k[2] + rho[mst][nst] * std::conj(F_S_fm) * F_S_fn;
                            res_k[2] = res_k[2] + rho[nst][mst] * F_S_mn;
                        }
                    }

                    // std::vector<complex_t> res_k(Nst);

                    // for (size_t i = 0; i < res_k.size(); i++)
                    // {
                    //     res[i] = std::norm(res_k[i]);
                    // }

                    return res_k[2];
                };

                // std::vector<std::complex<double>> res_(3, 0.);
                ...
                std::cout << "Ispot         : " << res << std::endl;

                diff_t_out << std::setw(25) << std::norm(0);

                diff_t_out << std::setw(25) << std::norm(0);

                diff_t_out << std::setw(25) << std::norm(res);
            }
        }
