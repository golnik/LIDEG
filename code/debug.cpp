...
int main(){
    ...
    auto tgrid=create_grid(tmin,tmax,Nt);
    for(auto time: tgrid){
        data.push_back(cos(time));
    }

    ExternalField* Ex=new ExternalFieldFromData(data,tmin,tgrid[1]-tgrid[0]);
    ExternalField* Ey=new ExternalFieldFromData(data,tmin,tgrid[1]-tgrid[0]);
    ExternalField* Ax=new ExternalFieldFromData(data,tmin,tgrid[1]-tgrid[0]);
    ExternalField* Ay=new ExternalFieldFromData(data,tmin,tgrid[1]-tgrid[0]);

    double e2p = 0.0;
    double g0  = 3.033/au2eV;
    double s0  = 0.129;
    double a   = 0.246/au2nm;

    /*GrapheneModel2 gm(a,e2p,g0,s0,Ex,Ey,Ax,Ay);

    //double res1=gm.chi_t(0.5,0.5,0.5);
    double res2=gm.dchidt(0.5,0.5,0.5);

    //std::cout<<res1<<" "<<res2<<std::endl;*/

    auto nfield=new NewField(*static_cast<ExternalFieldFromData*>(Ex));

    for(auto time: tgrid){
        auto const autodiff=(*nfield)(make_fvar<double,1>(time));

        std::cout<<time<<" ";
        
        std::cout<<(*nfield)(time)<<" ";
        std::cout<<autodiff.derivative(0)<<" ";
        std::cout<<autodiff.derivative(1)<<" ";
